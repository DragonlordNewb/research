...
class Metric:
	...
	const = lambda x: lambda location, displacement: x

	def __init__(self) -> None:
		self._spacetime: "Spacetime" = None
		if self.tensor is None:
			raise NotImplementedError("Can\'t use base Metric class.")
	...
	@spacetime.setter
	def spacetime(self, value: Any) -> None:
		self._spacetime = value
		if value is not None:
			if self not in self._spacetime:
				self._spacetime << self

	def __getitem__(self, index: tuple[int, int]) -> None:
		mu, nu = index
		return self.tensor[nu][mu]
	
	def uniaxial(self, axis: int, location: Vec3, displacement: Vec4) -> Scalar:
		gii = self[axis, axis]
		if self.spacetime is None:
			raise RuntimeError("Need to assign the metric to a Spacetime.")
		warp2 = gii(self.spacetime, location, displacement)
		return sqrt(warp2)

	def biaxial(self, mu: int, nu: int, location: Vec3, displacement: Vec4) -> tuple[Scalar, Scalar]:
		gmunu = self[mu, nu]
		if self.spacetime is None:
			raise RuntimeError("Need to assign the metric to a Spacetime.")
		warp = gmunu(self.spacetime, location, displacement)
		fdxmu = warp / displacement[nu]
		fdxnu = warp / displacement[mu]
		return (fdxmu, fdxnu)

	def warp(self, location: Vec3, displacement: Vec4) -> Vec4:
		w = Vector.zero(4)
		for axis in I_0123:
			w[axis] = self.uniaxial(axis, location, displacement)

		for mu, nu in I_0123_NONDIAG:
			wmu, wnu = self.biaxial(mu, nu, location, displacement)
			w[mu] *= wmu
			w[nu] *= wnu
...
class Particle:
	...
	def __init__(self, location: Vec3, mass: Scalar, **charges: dict[str, Scalar]) -> None:
		self.location, self.mass, self.charges = location, mass, charges
		self.velocity = Vector.zero(3)
		self.locked = False

		self.experiencedSpacetime = Vector.zero(4)
	...
	def displace(self, displacement: Vec4) -> None:
		for axis in I_0123:
			self.experiencedSpacetime[axis] += abs(displacement[axis])
...
class Force(ABC):
	...
	@abstractmethod
	def force(self, particle: Particle):
		pass
...
class Field(Force):
	...
	def force(self, particle: Particle) -> Scalar:
		return -self.calculus.gradient(lambda v: self.potential(particle, v), particle.location) * self.coupling(particle)

	@abstractmethod
	def potential(self, particle: Particle, location: Vec3) -> Scalar:
		pass

	@abstractmethod
	def coupling(self, particle: Particle) -> Scalar:
		pass
...
class Interaction(Force):
	...
	def force(self, particle: Particle) -> Scalar:
		f = Vector.zero(3)
		for otherParticle in spacetime.other(particle):
			f += self.interact(particle, otherParticle)
		return F

	@abstractmethod
	def interact(self, particle: Particle, otherParticle: Particle) -> Scalar:
		pass