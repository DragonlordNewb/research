...
class Field:
	...
	def __init__(self, minimum: Vec3, maximum: Vec3, resolution: Scalar=Decimal("1e-12"), generator: Callable[[Vec3], Any]=None):
		...
		self.locations = {
			x: {
				y: {
					z: generator(Vec3(x, y, z)) for z in range(minimum.z, maximum.z, resolution)
				} for y in range(minimum.y, maximum.y, resolution)
			} for x in range(minimum.x, maximum.x, resolution)
		}
